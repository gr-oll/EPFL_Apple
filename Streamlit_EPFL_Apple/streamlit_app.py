...
#  USER AUTH
# ──────────────────
USERS_FILE = "Streamlit_EPFL_Apple/users.json"
KEY_FILE = "Streamlit_EPFL_Apple/secret.key"
...
def check_password(password: str, hashed: str) -> bool:
    ...
    f = Fernet(key)
    try:
        return f.decrypt(hashed.encode()).decode() == password
    except Exception:
        return False
    ...
    with tab2:
        ...
        if st.button("Register"):
            if new_user in users:
                ...
                users[new_user] = {"password": encrypt_password(new_pass), "user_id": new_user_id}
                save_users(users)
